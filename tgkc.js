...
class VideoBot {
	...
	updateCooldownTimer(adminId, state) {
		...
		state.timer = setTimeout(async () => {
			try {
				...
				] = await this.pool.query('SELECT COUNT(*) as count FROM videos');
				this.bot.sendMessage(
					state.chatId,
					`[管理员 ${adminId}]您好，已成功收录 ${state.count} 个视频，当前总库存：${count}`, this.config
					.uploadVideosInfos
				);
    ...
    `);
			...
			await this.bot.sendMessage(chatId, `✅ 视频去重完成，去重后库存${count}个视频！`);
	...
	async handleAdminDown(chatId) {
		try {
			...
			await new Promise((resolve, reject) => {
				exec(
					`mysqldump -h${host} -u${user} -p${password} ${database} videos > ${videoDump}`,
			...
			await new Promise((resolve, reject) => {
				...
				archive.pipe(output);
				archive.file(videoDump, {
					name: 'videos.sql'
				});
				archive.file(groupDump, {
					name: 'groups.sql'
				});
				archive.finalize();
			});
			...
			logger.info(`管理员${chatId}下载了视频表和群组表`);

			// 删除临时文件
			await fs.unlink(videoDump);
			await fs.unlink(groupDump);
